...
export async function resolveEndUser(c: Context, project: PublicProject) {
  const jwt = c.req.header('x-heed-user')
  if (jwt) {
    try {
  ...
  const uid = await getSignedCookie(c, env.HEED_AUTH_SECRET, COOKIE)
  if (uid) {
...
export async function requireEndUser(c: Context, project: PublicProject) {
  ...
  await setSignedCookie(c, COOKIE, anon.id, env.HEED_AUTH_SECRET, {
    httpOnly: true,
    sameSite: 'None',
    secure: true,
    path: '/',
